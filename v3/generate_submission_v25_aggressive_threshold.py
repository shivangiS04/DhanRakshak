...
Test even more aggressive threshold: 0.38
Expected: 1200+ windows detected
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('submission_v25_aggressive_threshold.log'),
...
def add_fraud_detection_features(X):
    """Add burstiness and flow_balance features"""
    X_new = X.copy()
    
    X_new['burstiness'] = X['total_transactions'] / (X['transaction_span_days'] + 1)
    X_new['flow_balance'] = np.abs(X['total_credit_amount'] - X['total_debit_amount'])
    X_new['flow_balance_ratio'] = X_new['flow_balance'] / (X['total_credit_amount'] + X['total_debit_amount'] + 1e-8)
...
def train_fast_ensemble(X_train, y_train, X_val, y_val):
    ...
    logger.info("Training fast ensemble...")
...
def generate_smart_window(prob: float, features_row: dict, account_id: str = None) -> tuple:
    """Generate smart windows with threshold 0.38"""
...
def main():
    logger.info("=" * 80)
    logger.info("GENERATING SUBMISSION V25 — Aggressive Threshold (0.38)")
    logger.info("=" * 80)
    ...
    try:
        ...
        X_tr = add_fraud_detection_features(X_tr)
        X_te = add_fraud_detection_features(X_te)
        ...
        logger.info("Generating smart temporal windows...")
        ...
        for idx, row in features_for_windows.iterrows():
            ...
            start_str, end_str = generate_smart_window(prob, features_row, account_id)
        ...
        output_dir = Path('output_v25_aggressive_threshold')
        output_dir.mkdir(exist_ok=True)
        output_path = output_dir / 'submission_v25_aggressive_threshold.csv'
        ...
        logger.info(f"  Accounts flagged (prob > 0.38): {(submission['is_mule'] > 0.38).sum()}")
